#include "mapwidget.h"
#include <QFile>
#include <QDir>
#include <QTextSTream>

MapWidget::MapWidget(QWidget *parent) :
    QWidget(parent)
{
    WIDTH=80;
    HEIGHT=80;
    int WdSize=20;
    QPalette pe;
    pe.setColor(QPalette::WindowText,"#ffffff");
    QFont font ( "Microsoft YaHei",WdSize, 60);//Size,B

    importButton=(new QPushButton(this));
    importButton->setGeometry(10,10,1.2*WIDTH,1.2*HEIGHT);
    importButton->setEnabled(true);
    importButton->setIcon(QIcon(":/res/download00.png"));
    importButton->setIconSize(QSize(1.2*WIDTH,1.2*HEIGHT));
    importButton->setFlat(true);

    next_step_bt=(new QPushButton(this));
    next_step_bt->setEnabled(true);
    next_step_bt->setIcon(QIcon(":/res/nextstep.png"));
    next_step_bt->setIconSize(QSize(1.2*WIDTH,1.2*HEIGHT));
    next_step_bt->setFlat(true);
    next_step_bt->setHidden(true);
    //next_step_bt->setStyleSheet("border: 0px");//消除边框，取消点击效果
    next_step_lb=(new QLabel(this));
    next_step_lb->setText("下一步");
    next_step_lb->setFont(font);
    next_step_lb->setPalette(pe);
    next_step_lb->setEnabled(true);
    next_step_lb->setHidden(true);


    scaleUP =new QPushButton(this);
    scaleUP->setEnabled(true);
    scaleUP->setIcon(QIcon(":/res/zoomin.png"));
    scaleUP->setIconSize(QSize(WIDTH,HEIGHT));
    scaleUP->setFlat(true);
    scaleDown =new QPushButton(this);
    scaleDown->setEnabled(true);
    scaleDown->setIcon(QIcon(":/res/zoomout.png"));
    scaleDown->setIconSize(QSize(WIDTH,HEIGHT));
    scaleDown->setFlat(true);
    posing =new QPushButton(this);
    posing->setEnabled(true);
    posing->setIcon(QIcon(":/res/posing.png"));
    posing->setIconSize(QSize(WIDTH+30,HEIGHT+30));
    posing->setFlat(true);

    NavigationBox =new QGroupBox(this);
    //NavigationBox->setCheckable(true);
    NavigationBox->setChecked(false);
    NavigationBox->setFlat(true);
    NavigationBox->setFont(font);
    NavigationBox->setPalette(pe);

    int w=300,h=60;
    navi_bt=(new QPushButton(NavigationBox));
    navi_bt->setEnabled(true);
    navi_bt->setGeometry(0,20,w/3,2*h);
    navi_bt->setIcon(QIcon(":/res/navi.png"));
    navi_bt->setIconSize(QSize(w/3,2*h));
    navi_bt->setFlat(true);
    start_select_bt =(new QPushButton(NavigationBox));
    start_select_bt->setGeometry(w/3,20,w/4,h);
    start_select_bt->setEnabled(false);
    start_select_bt->setText("起点");
    start_select_bt->setIconSize(QSize(w/4,h/2));
    start_select_bt->setFlat(true);
    startpos=new QLineEdit(NavigationBox);
    startpos->setGeometry(w/3+w/4,20,1.6*w,h);
    startpos->setEnabled(false);

    end_select_bt =(new QPushButton(NavigationBox));
    end_select_bt->setGeometry(100,20+h,w/4,h);
    end_select_bt->setEnabled(false);
    end_select_bt->setText("终点");
    end_select_bt->setIconSize(QSize(w/4,h/2));
    end_select_bt->setFlat(true);
    endpos=new QLineEdit(NavigationBox);
    endpos->setGeometry(w/3+w/4,20+h,1.6*w,h);
    endpos->setEnabled(false);

    this->setAttribute(Qt::WA_AcceptTouchEvents);//接收touch消息
    //
    connect(importButton,SIGNAL(clicked(bool)),this,SLOT(OpenMapFile()));
    connect(scaleUP,SIGNAL(clicked(bool)),this,SLOT(scaleup()));
    connect(scaleDown,SIGNAL(clicked(bool)),this,SLOT(scaledown()));
    connect(posing,SIGNAL(clicked(bool)),this,SLOT(posingcenter()));
    //connect(NavigationBox,SIGNAL(toggled(bool)),this,SLOT(navigationshow(bool)));
    connect(navi_bt,SIGNAL(clicked(bool)),this,SLOT(navigationshow(bool)));

    //viewinit();
    //初始默认路径加载Map
    OpenMapFile();

}

MapWidget::~MapWidget()
{
}

void MapWidget::viewinit()
{

    scale=2;
    movedpos=QPoint(0,0);
    moveingpos=QPoint(0,0);
    selectflag=1;
    startpos->setText("单击选择起点...");
    endpos->clear();
}

void MapWidget::paintEvent(QPaintEvent *){

       QPainter painter(this);
       painter.setBrush(Qt::black);
       painter.drawRect(this->rect());

       NavigationBox->setGeometry(this->width()-600-20,10,600,200);
       scaleUP->setGeometry(this->width()-WIDTH-20,this->height()-3*HEIGHT-20,WIDTH,HEIGHT);
       scaleDown->setGeometry(this->width()-WIDTH-20,this->height()-2*HEIGHT-20,WIDTH,HEIGHT);
       posing->setGeometry(this->width()-WIDTH-20,this->height()-HEIGHT-20,WIDTH,HEIGHT);
       next_step_bt->setGeometry(10,this->height()-HEIGHT-20,1.2*WIDTH,1.2*HEIGHT);
       next_step_lb->setGeometry(10+1.2*WIDTH,this->height()-HEIGHT-20,1.2*WIDTH,1.2*HEIGHT);

       //Draw roadNet
       for(int i=0;i<wayList.size();i++)
       {
           Way way=wayList.at(i);
           painter.setPen(QPen(Qt::darkGray, 3));
           for(int j=1;j<way.ndlist.size();j++)
           {
               Node node1=nodeHash.value(way.ndlist.at(j-1));
               Node node2=nodeHash.value(way.ndlist.at(j));
               painter.drawLine(QPointF(geo2window(node1)),QPointF(geo2window(node2)));
           }
       }

       //Draw Nodes
       QList<Node> nodeList=nodeHash.values();
       QPointF *points=new QPointF[nodeList.size()];
       for(int i=0;i<nodeList.size();i++)
       {
           Node node=nodeList.at(i);
           points[i]=QPointF(geo2window(node));
       }
       painter.setPen(QPen(Qt::green, 2));
       painter.drawPoints(points,nodeList.size());
       delete points;

       //Draw path
       painter.setPen(QPen(Qt::blue, 4));
       if(!path.empty()&&(selectflag!=2))
           {
           auto p=path.front();
           Node node1;Node node2;
           node1.lon=p->x;
           node1.lat=p->y;
           QPoint pos1=geo2window(node1);
           for (auto &p : path)
           {
             node2.lon=p->x;
             node2.lat=p->y;
             QPoint pos2=geo2window(node2);
             painter.drawLine(pos1,pos2);
             pos1=pos2;
           }

           QPoint startpos=geo2window(start);
           painter.drawPixmap(startpos.x()-15,startpos.y()-50,30,50, QPixmap(":/res/pos_s.png"));
           QPoint endpos=geo2window(end);
           painter.drawPixmap(endpos.x()-15,endpos.y()-50,30,50, QPixmap(":/res/pos_e.png"));
           }

       //Start point
       if(selectflag==2)
       {
           QPoint startpos=geo2window(start);
           painter.drawPixmap(startpos.x()-15,startpos.y()-50,30,50, QPixmap(":/res/pos_s.png"));
       }

       //Test
       painter.setPen(QPen(Qt::red, 4));
       painter.drawPoint(geo2window(geocenter));

}

void MapWidget::OpenMapFile()
{

    if(filename.isEmpty())//初始
        filename=QDir::currentPath()+"/HDmaps/HDmap_0.XML";
    else
    {
        //qDebug()<<filename;
        QString this_filename = QFileDialog::getOpenFileName(this,tr("OpenFile"),"","VCI_hdMap(*.xml;*.XML);;OpenStreetMap(*.osm;*.OSM)");
        if(this_filename.isEmpty())
            return;
        filename=this_filename;
    }

    OSMparser se(filename);
    bounds=se.bounds;
    nodeHash=se.nodeHash;
    wayList=se.wayList;
    points=se.points;
    //se.ShowOSM();//????????????????????????????
    geocenter.lon=(bounds.minlon+bounds.maxlon)/2;
    geocenter.lat=(bounds.minlat+bounds.maxlat)/2;

    viewinit();

    if(wayList.size()>2)//地图加载成功
    {
        importButton->setHidden(true);
    }
}

void MapWidget::mousePressEvent(QMouseEvent *event)
{
    presspos=event->pos();
    //if(!NavigationBox->isChecked())
    //   return;

}

void MapWidget::mouseMoveEvent(QMouseEvent *event)
{
    moveingpos=movedpos+(event->pos()-presspos)/scale;
    update();
}

void MapWidget::mouseReleaseEvent(QMouseEvent *event)
{
    if(event->pos()==presspos)
    {
        if(selectflag==1)
        {//select start node
            start=getnearstpoint(window2geo(presspos));
            startpos->setText("<"+QString::number(start.lon,10,7)+","+QString::number(start.lat,10,7)+">");
            endpos->setText("单击选择终点...");
            selectflag=2;
        }
        else if(selectflag==2)
        {//select end node
            end=getnearstpoint(window2geo(presspos));
            endpos->setText("<"+QString::number(end.lon,10,7)+","+QString::number(end.lat,10,7)+">");
            Astar astar;
            astar.InitAstar(points);
            Point s("",start.lon,start.lat);
            Point e("",end.lon,end.lat);
            path.clear();
            path = astar.GetPath(s,e, false);
            //writepath2mfile(path);
            next_step_bt->setHidden(false);//完成规划，可以进入下一步
            next_step_lb->setHidden(false);
            selectflag=3;
        }
    }

    movedpos+=(event->pos()-presspos)/scale;
    update();
}

void MapWidget::mouseDoubleClickEvent(QMouseEvent *event)
{
    //qDebug()<<"DoubleClick on:"<<event->pos();
    Node node=window2geo(event->pos());
    //qDebug()<<node.lat<<node.lon;
    //qDebug()<<geo2window(node);
    setgeocenter(node.lon,node.lat);
}

void MapWidget::scaleup()
{
    if(scale<20)
    {
        scale=2*scale;
        this->repaint();
    }
}

void MapWidget::scaledown()
{
    if(scale>0.01)
    {
    scale=scale/2;
    this->repaint();
    }
}

QPoint MapWidget::geo2window(Node node)
{
    double dlat=node.lat-geocenter.lat;
    double dlon=node.lon-geocenter.lon;
    double x=dlon*1.1132e+05*cos(0.01745*geocenter.lat);
    double y=-dlat*1.1132e+05;
    x=x+moveingpos.x();
    y=y+moveingpos.y();
    return QPoint(x*scale+this->width()/2,y*scale+this->height()/2);
}

Node MapWidget::window2geo(QPoint pos)
{
    double x=pos.x()-this->width()/2;
    double y=pos.y()-this->height()/2;
    x=x/scale;
    y=y/scale;
    x=x-moveingpos.x();
    y=y-moveingpos.y();
    double dlat=-y/1.1132e+05;
    double dlon=x/(1.1132e+05*cos(0.01745*geocenter.lat));
    Node node;
    node.lat=dlat+geocenter.lat;
    node.lon=dlon+geocenter.lon;
    return node;
}

void MapWidget::setgeocenter(double lon, double lat)
{
    geocenter.lon=lon;
    geocenter.lat=lat;
}

 void MapWidget::posingcenter()
 {
     Node node;
     node.lon=geocenter.lon;
     node.lat=geocenter.lat;
     QPoint setpos=geo2window(node);
     QPoint wincenter=QPoint(this->width()/2,this->height()/2);
     moveingpos+=(wincenter-setpos)/scale;
     movedpos+=(wincenter-setpos)/scale;
     //scale=2;
     this->repaint();
 }

void MapWidget::navigationshow(bool s)
{
       /*
        start_select_bt->show();
        startpos->show();
        end_select_bt->show();
        endpos->show();
        */
        path.clear();
        next_step_bt->setHidden(true);
        next_step_lb->setHidden(true);

        if(selectflag==0)
        {
            selectflag=1;
            startpos->setText("单击选择起点...");
        }
        else if(selectflag==3)
        {
            selectflag=1;
            startpos->setText("单击选择起点...");
            endpos->setText("");
            path.clear();
            OSMparser se(filename);
            bounds=se.bounds;
            nodeHash=se.nodeHash;
            wayList=se.wayList;
            points=se.points;
        }
        this->repaint();
}

Node MapWidget::getnearstpoint(Node node)
{
    Node nearstnode=node;
    double dicetanceThreshold=1000;
    QList<Node> nodeList=nodeHash.values();
    for(int i=0;i<nodeList.size();i++)
    {
        Node standnode=nodeList.at(i);
        QPoint standwp=geo2window(standnode);
        QPoint nodewp=geo2window(node);
        double distance=(standwp-nodewp).manhattanLength();
        if(dicetanceThreshold>distance)
        {
            dicetanceThreshold=distance;
            nearstnode=standnode;
        }
    }
    //qDebug()<<nearstnode.lon<<nearstnode.lat;

    return nearstnode;
}

bool MapWidget::writepath2mfile(std::list<Point *> path)
{

    QFile file("hdmap.m");   //Write2filename.m
    if(!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate))
        return false;
    QTextStream out(&file);
    QString lonarray,latarray,altarray,lspeedarray;
    for (auto &p : path)
    {
        lonarray=lonarray+tr(" ")+QString::number(p->x,10,7);
        latarray=latarray+tr(" ")+QString::number(p->y,10,7);
        altarray=altarray+tr(" ")+QString::number(200,10,0);
        lspeedarray=lspeedarray+tr(" ")+QString::number(p->lspeed,10,0);
    }
    qDebug()<<tr("Write mfile:");
    qDebug()<<lonarray;
    qDebug()<<latarray;
    qDebug()<<altarray;
    qDebug()<<lspeedarray;

    out<<tr("% -------------------------------------------------------------------")<<endl;
    out<<tr("%Generated by RoadMapv1.0 on 2018")<<endl;
    out<<tr("%VCIer you can contact us by sending email:2580253221@qq.com")<<endl;
    out<<tr("% -------------------------------------------------------------------")<<endl<<endl;
    out<<tr("HDmap = struct;")<<endl;
    out<<tr("HDmap.lonarray = [")<<lonarray<<tr("];")<<endl;
    out<<tr("HDmap.latarray = [")<<latarray<<tr("];")<<endl;
    out<<tr("HDmap.altarray = [")<<altarray<<tr("];")<<endl;
    out<<tr("HDmap.lspeedarray = [")<<lspeedarray<<tr("];")<<endl;

    file.close();
    return true;
}

void MapWidget::setRoadPointNo(int No)
{
  if(path.empty())//导航待开始
  {
      oldPointNo=-1;
      emit this->getRoadPointNo(-1);
      return;
  }
  else if(oldPointNo==No)return;
  else if(path.size()<No||0>No)//导航结束
  {
      No=path.size();
      emit this->getRoadPointNo(-2);
  }
  else
  {
  emit this->getRoadPointNo(No);
  int i=0;
  for (auto &p : path)
  {
      if(i==No)
      {
          emit this->getRoadPointLon(p->x);
          emit this->getRoadPointLat(p->y);
          emit this->getRoadPointAlt(233);
          emit this->getRoadPointLspeed(p->lspeed);
          //Node node;
          //node.lon=p->x; node.lat=p->y;
          //setgeocenter(p->x,p->y);
          break;
      }
      i++;
  }
  }
  oldPointNo=No;

}


void MapWidget::touchEvent(QTouchEvent *event)
{
    qDebug()<<event->type();
    switch (event->type()) {
       case QEvent::TouchBegin:
       {
           qDebug()<<"touch Begin";
       }
       case QEvent::TouchUpdate:
       {
           qDebug()<<"touch Update";
       }
       case QEvent::TouchEnd:
       {
           qDebug()<<"touch end";
       }
    default:
            qDebug()<<"touch default";
    }
}
